    ...
    return (
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
         <div className="bg-white rounded-2xl p-8 shadow-xl flex flex-col items-center">
            <div className="h-8 w-8 rounded-full border-4 border-slate-200 border-t-red-600 animate-spin mb-4"></div>
      ...
      <div className="bg-white shadow-2xl w-full max-w-md h-full flex flex-col overflow-hidden animate-in slide-in-from-right duration-300 border-l border-slate-200 min-w-0">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
          <div>
            <h2 className="font-bold text-slate-800 text-lg flex items-center gap-2">
              <User className="h-5 w-5 text-red-600"/> {data.name}
            </h2>
            <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-slate-200 text-slate-700 mt-1 inline-block">
        ...
        <div className="flex border-b border-slate-200 bg-white shrink-0">
          <button 
            ...
            className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'details' ? 'border-red-600 text-red-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            Lead Details
          </button>
          <button 
            onClick={() => setActiveTab('timeline')}
            className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'timeline' ? 'border-red-600 text-red-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            Timeline & Chat
          </button>
        </div>
                ...
                {previewTemplate ? (
                  <div className="border border-slate-200 rounded-xl p-4 bg-slate-50 animate-in fade-in min-w-0 overflow-hidden">
                    ...
                    <button 
                      ...
                      disabled={isSending}
                      className="w-full bg-slate-900 text-white font-bold text-sm py-2 rounded-lg hover:bg-slate-800 disabled:opacity-50 transition-colors"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button 
                      ...
                      className="flex flex-col items-center gap-3 p-6 rounded-2xl border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50 transition-all group"
                    >
                      ...
                      <span className="font-bold text-slate-700 group-hover:text-blue-700">Send Email</span>
                    </button>
                    <button 
                      onClick={() => setSelectedChannel('WHATSAPP')}
                      className="flex flex-col items-center gap-3 p-6 rounded-2xl border-2 border-slate-200 hover:border-emerald-500 hover:bg-emerald-50 transition-all group"
                    >
                      <div className="bg-emerald-100 p-3 rounded-xl group-hover:bg-emerald-500 transition-colors">
                        <MessageSquare className="h-8 w-8 text-emerald-600 group-hover:text-white" />
                      </div>
                      <span className="font-bold text-slate-700 group-hover:text-emerald-700">Send WhatsApp</span>
                    </button>
                  </div>
                ) : (
